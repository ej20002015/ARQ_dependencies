# This script uses vcpkg to build OpenSSL (Debug/Release, Shared)
...
Write-Host "--- Installing OpenSSL using vcpkg (Triplet: $vcpkgTriplet)... ---" -ForegroundColor Yellow
# This command builds both Debug and Release shared libraries by default
& "$VcpkgRootDir\vcpkg.exe" install "openssl:$vcpkgTriplet" --recurse
...
Copy-Item -Path "$vcpkgDebugLibDir\libcrypto.lib" -Destination "$installDir\debug\lib\" -Force
Copy-Item -Path "$vcpkgDebugLibDir\libssl.lib" -Destination "$installDir\debug\lib\" -Force
...
Copy-Item -Path "$vcpkgDebugBinDir\libcrypto*.dll" -Destination "$installDir\debug\bin\" -Force
Copy-Item -Path "$vcpkgDebugBinDir\libssl*.dll" -Destination "$installDir\debug\bin\" -Force
Copy-Item -Path "$vcpkgDebugBinDir\legacy*.dll" -Destination "$installDir\debug\bin\" -Force
# Copy PDBs if they exist
...
Copy-Item -Path "$vcpkgReleaseLibDir\libcrypto.lib" -Destination "$installDir\release\lib\" -Force
Copy-Item -Path "$vcpkgReleaseLibDir\libssl.lib" -Destination "$installDir\release\lib\" -Force

Write-Host "--- Copying Release binaries (.dll)... ---" -ForegroundColor Yellow
Copy-Item -Path "$vcpkgReleaseBinDir\libcrypto*.dll" -Destination "$installDir\release\bin\" -Force
Copy-Item -Path "$vcpkgReleaseBinDir\libssl*.dll" -Destination "$installDir\release\bin\" -Force
Copy-Item -Path "$vcpkgReleaseBinDir\legacy*.dll" -Destination "$installDir\release\bin\" -Force

# --- 6. Copy Custom CMake Module ---
Write-Host "--- Copying custom openssl.cmake module... ---" -ForegroundColor Yellow
...
Copy-Item -Path $cmakeModulePath -Destination "$installDir\cmake\openssl.cmake" -Force
...
Write-Host "`n--- Creating the OpenSSL distributable package (using 7-Zip)... ---" -ForegroundColor Yellow
$parentDir = (Get-Item -Path $installDir).Parent.FullName
$dirName = (Get-Item -Path $installDir).Name
Push-Location -Path $parentDir
& 7z.exe a -tzip -r "$packagePath" "$dirName" # Use 7zip for potentially large PDBs
Pop-Location

Write-Host "`n--- OpenSSL Package Complete! ---" -ForegroundColor Green
